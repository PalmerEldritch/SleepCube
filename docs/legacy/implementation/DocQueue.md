...
| `sc_light_service_*` | Light state, persistence, and animation orchestration | Documented | `promote: light-engine` | Firmware | See `SC_P0_LightingEngine.md`; brightness is restored from NVS and LCD ambience now follows shared brightness/audio state using backlight PWM rendering |
...
| `sc_app_core_*` | Service startup + event dispatch | Documented | `promote: runtime-core` | Firmware | See `SC_P0_RuntimeServices.md`; primary UI model is ambient rest mode with tap-to-toggle audio plus long-press slider reveal for volume/brightness |
