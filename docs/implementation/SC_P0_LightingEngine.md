...
- Included:
  ...
  - light service state model (enabled flag, target brightness, time-based settling)
  - persisted brightness restore at startup
  - startup fade-in with gentle bloom
  - global ambient fluctuation (brightness + warmth) tuned for low-distraction motion
  - soft spatial gradient across the LED strip
  - one-shot audio transition pulse on audio state transitions
...
3. Each loop:
   - settles `current` brightness toward `target` with a time-based response
   - applies startup envelope
   - applies ambient fluctuation
   - applies optional audio transition pulse envelope
...
`target_brightness` -> `current_brightness` -> `startup_scale` -> `+ subtle fluctuation` -> `+ audio transition pulse` -> clamp [0..100] -> light engine:

- perceptual brightness (`gamma 2.2`)
- max brightness cap (`CONFIG_SC_LED_MAX_BRIGHTNESS_PCT`)
- slight warmth shift blend
- soft strip-wide spatial gradient
...
| `sc_light_service_audio_sway(audio_enabled)` | bool | `esp_err_t` | Injects brighten/dim transition pulse for audio on/off |
...
| `CONFIG_SC_LED_STRIP_GPIO` | `8` on Waveshare, `13` on DevKitC | Data output pin |
...
| `CONFIG_SC_LIGHT_RAMP_STEP_PCT` | `2` | Legacy step tuning input; runtime behavior now uses a time-based settle for calmer response |
...
| `CONFIG_SC_LIGHT_FLUCT_BRIGHTNESS_PCT` | `6` | Upper bound for subtle fluctuation amplitude (brightness) |
| `CONFIG_SC_LIGHT_FLUCT_WARMTH_PCT` | `10` | Upper bound for subtle fluctuation amplitude (warmth) |
| `CONFIG_SC_LIGHT_SPATIAL_UNDULATION_ENABLE` | `y` | Enables strip-wide spatial movement |
| `CONFIG_SC_LIGHT_SPATIAL_UNDULATION_PCT` | `12` | Upper bound for soft spatial gradient amplitude |
| `CONFIG_SC_LIGHT_SPATIAL_KNOTS` | `4` | Spatial control points |
| `CONFIG_SC_LIGHT_SPATIAL_SPEED_PCT` | `70` | Spatial motion speed input; implementation scales this down for calmer movement |
| `CONFIG_SC_LIGHT_AUDIO_SWAY_ENABLE` | `y` | Audio sway switch |
| `CONFIG_SC_LIGHT_AUDIO_SWAY_PCT` | `8` | Audio transition pulse amplitude input |
| `CONFIG_SC_LIGHT_AUDIO_SWAY_MS` | `1400` | Audio transition pulse duration |
...
- LED steady-state motion is intentionally kept barely noticeable; the strip should read more like a calm lamp than an animated effect.
...
- Visual checks:
  ...
  - startup bloom is subtle rather than theatrical
  - fluctuation is gentle and continuous
  - strip reads mostly uniform with only a soft gradient
  - audio on/off triggers a short brighten/dim pulse without ongoing audio-linked motion
